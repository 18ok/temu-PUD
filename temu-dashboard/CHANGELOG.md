...
- **商品图片能力**：利润统计支持 `图片/主图/图片链接` 与 `差评图/评价图片/买家秀` 列；商品画像新增图片墙，避坑清单新增问题品图片与评价图入口
- **产品定位收口**：界面与文档统一为 `跨境经营诊断系统 · Temu 子系统`，侧栏预留公司经营总览、SHEIN、淘宝天猫入口，明确 Temu 是试点子系统而非公司主系统成品
- **侧栏层级重构**：公司经营总览提到首位；Temu / SHEIN / 淘宝天猫归入平台子系统；现有 8 个业务页面嵌入 `Temu 子系统` 卡片内的 `具体功能`，形成公司总览 -> 平台子系统 -> 子系统功能的三级结构，平台列表支持内部滚动
- **公司经营总览启用**：新增老板/组长视角一级入口，展示小组成绩、平台状态、图片数据质量和协作服务状态，不暴露个人原始明细
- **图片数据质量闭环**：商品画像补齐主图覆盖率、评价图覆盖率、待补主图数量；同步摘要新增 `data_quality.images` 并保留 `image_url` / `review_image_url`
- **管理后台商业化边界**：新增普通运营、组长/主管、管理员、开发者四档权限矩阵，并增加 OSS、ECS/轻量服务器、协作数据库、域名 HTTPS 部署链路检查
- **阿里云部署路线**：新增 `docs/阿里云部署路线V1.md`，明确本机试点 -> 阿里云 MVP -> 正式协作版，推荐单 ECS/轻量服务器 + `api_server.py` + SQLite + OSS 起步
